...
export function Card({ product }) {
    ...
    return (
        <>
            <div className='m-1'>
                <div class="flex flex-wrap z-10  mt-2 bg-zinc-300 dark:bg-zinc-700 rounded-xl drop-shadow-lg ">
                    <div class=" w-full lg:max-w-full lg:flex transition duration-300 dark:hover:bg-zinc-800 rounded-2xl">
                        <div class=" lg:border-l-0  rounded-xl p-4  leading-normal">
                            <button onClick={() => {
                                openModal();
                                handleView();
                            }}>
                                <div class="mb-8">
                                    <p class="text-sm text-gray-900 dark:text-gray-100 flex ">
                                    ...
                                    <div class="text-gray-900 dark:text-gray-100 flex  font-bold text-xs md:text-base mb-2">{product.name.charAt(0).toUpperCase() + product.name.slice(1)}</div>
                                </div>
                                <div class="flex justify-start ">
                                    ...
                                    <div class="text-sm text-left">
                                        <p class="text-gray-900 dark:text-gray-100 flex text-xs  leading-none">{product.userEmail && product.userEmail.split('@')[0]}</p>
                                        <p class="text-gray-900 dark:text-gray-100 flex  text-xs  "> {product.timestamp}</p>
                                    </div>
                                </div>
                            </button>
                            <div className='flex justify-between m-4' >
                                <button type="button" class="text-gray-900 dark:text-gray-100 flex   hover:bg-zinc-200 dark:hover:bg-zinc-900  font-medium rounded-lg text-xs md:text-sm py-2 px-4 text-center inline-flex items-center me-2 ">
                                ...
                                <button onClick={handleLike} type="button" class="text-gray-900 dark:text-gray-100 flex hover:bg-zinc-200 dark:hover:bg-zinc-900 font-medium rounded-lg text-sm py-2 px-4 text-center inline-flex items-center me-22">
                                    <svg class={`w-4 h-4 md:w-6 md:h-6 ${userLiked ? 'text-yellow-600' : ''}`} aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 18 18">

...
import { ViewHome } from "../../pages/Matriz/ViewHome";
...
export function Card({ product }) {
    ...
    useEffect(() => {
        ...
        const likesQuery = query(collection(db, 'likesMatriz'),
    ...
    const handleLike = async () => {
        ...
        if (userLiked) {
            ...
            const q = query(collection(db, 'likesMatriz'),
                where('userId', '==', user.uid),
                where('productId', '==', product.id));
            ...
            await updateDoc(doc(db, 'Matrices', product.id), {
                like: like - 1
            });
        } else {
            // Si el usuario no ha dado like, agregar el like
            await addDoc(collection(db, 'likesMatriz'), {
                userId: user.uid,
    ...
    return (
        <>
            <div className='m-1'>
                <div class="flex flex-wrap z-10  mt-1 bg-zinc-300 dark:bg-stone-900 rounded-xl drop-shadow-lg ">

...
export function Edit({ closeModal, postId }) {
    ...
    const getPostsById = async (postId) => {
        const post = await getDoc(doc(db, "places", postId))
        if (post.exists()) {
    ...
    const update = async (e) => {
        e.preventDefault()
        const post = doc(db, "places", postId)
    ...
    return (
        <div className=" fixed inset-0 flex items-center justify-center z-50">
            ...
            <div class="fixed inset-0z-10 w-screen overflow-y-auto">
                        ...
                        <form onSubmit={update} className="p-6">
                            <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-full">
                                ...
                                <div class=" col-start-1 col-end-7 md:col-start-4 col-end-7 justify-items-center">
                                    <img src={file ? URL.createObjectURL(file) : downloadURL} className="justify-center h-24 w-24 md:h-25 md:w-30 rounded-md object-cover" />
                                </div>

...
export function Places() {
  ...
  const deletePost = async (id) => {
    const deletePots = doc(db, "places", id)
        ...
        <div class="wrapper mt-4">
          <div className='bg-white shadow-[0_8px_12px_-6px_rgba(0,0,0,0.2)] border p-6 w-full  rounded-lg font-[sans-serif] overflow-hidden mx-auto my-4'>
            <div className='flex  py-6 px-6 rounded-xl justify-between'>
              ...
              <div className='flex '>
                <Link to="/places/new-place"> <button className='button text-white mr-2' >+ new place</button></Link>
              </div>
            </div>
                ...
                <tbody>
                  {Array.isArray(posts) && posts.map((post) => (
                    <tr key={post.id} class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                      ...
                      <td class="px-6 py-4">
                        <div className='flex'>
                          <a href="#" class=" flex font-medium bg-green-600 p-1 mr-1 rounded-md hover:bg-green-900 text-slate-50" onClick={() => openModal(post.id)}>
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="font-bold  w-5 h-5">
                              <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                            </svg>
                          </a>
                          <a href="#" onClick={() => deletePost(post.id)} class=" flex font-medium bg-red-600 p-1 rounded-md hover:bg-red-800 text-zinc-50 ">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" className="w-5 h-5 font-bold ">

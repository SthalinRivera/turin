        ...
        <main className="flex-1 p-2 md:p-6 bg-gray-100 dark:bg-slate-800">
            <div className="bg-white dark:bg-slate-900 shadow-lg  p-2 md:p-4 rounded-lg font-sans">
                ...
                <div className="flex justify-between items-center mb-4">
                    <h1 className="text-lg md:text-xl font-bold">Reporte de satisfacción de usuarios</h1>
                    <button onClick={refreshData} className="button text-white text-sm md:text-base px-3 py-2 rounded hover:bg-blue-700 transition flex items-center">
                        <FaRedo />
                                ...
                                <Pie
                                    ...
                                    options={{
                                        ...
                                        plugins: {
                                            ...
                                            title: {
                                                display: true,
                                                text: questions[qKey], // Texto real de la pregunta
                                            },
                                        },
                                    }}
                                />
            ...
            <div className="bg-white dark:bg-slate-900 shadow-lg  p-6 rounded-lg">
                    ...
                    <>
                        <table className="table-auto w-full mb-6">
                            <thead>
                                <tr>
                                    {questionOrder.map((qKey) => (
                                        <th key={qKey} className="border px-4 py-2">
                                            {questions[qKey]} {/* Muestra el texto de la pregunta */}
                                        </th>
                                    ))}
                                </tr>
                            </thead>
                            <tbody>
                                {surveys.map((survey, index) => (
                                    <tr key={index}>
                                        {questionOrder.map((qKey) => (
                                            <td key={qKey} className="border px-4 py-2 text-center">
                                                {survey[qKey]}
                                            </td>
                                        ))}
                                    </tr>
